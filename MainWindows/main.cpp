...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, INT nCmdShow)
{
	...
	(
		...
		WS_OVERLAPPEDWINDOW, //стиль окна OVERLAPPED всегда задается для главного окна
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE: //создаются элементы окна
		break;
	case WM_COMMAND: //обработка команд нажатия кнопок и др элементов окна
		break;
	case WM_SIZE:
	case WM_MOVE:
	{
		...
		sprintf(buffer, "%s Position: %dx%d, Sise: %dx%d", g_szTitle, rect.left, rect.top, rect.right-rect.left, rect.bottom-rect.top);
			//%s - строка
			//%d - десятичное число
